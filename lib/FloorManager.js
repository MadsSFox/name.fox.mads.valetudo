...
class FloorManager {
  ...
  async removeFloor(id) {
    // Remove robot files first — if this fails, abort before touching the store
  ...
  async _saveMapFiles(floorDir) {
    ...
    // robot.db — only backup if we successfully saved at least one map file
    if (savedCount > 0) {
  ...
  async _waitForOnline() {
    ...
    // Phase 2: Wait for Valetudo to load the map (layers + segments present).
    // After reboot the SLAM may first serve floor/wall layers before it has
    // finished loading segment assignments. We wait until segment layers are
    // present OR until a stabilisation timeout expires after layers first appear.
    const mapWaitMs = 60000; // up to 60s total
    const STABILIZE_MS = 5000; // minimum extra wait after layers first detected
    const mapStart = Date.now();
    let layersSeenAt = null;
    while (Date.now() - mapStart < mapWaitMs) {
      try {
        const mapData = await this._api.getMap();
        if (mapData && mapData.layers && mapData.layers.length > 0) {
          if (!layersSeenAt) {
            this._log('Layers detected, waiting for SLAM to stabilise...');
            layersSeenAt = Date.now();
          }
          const hasSegments = mapData.layers.some((l) => l.type === 'segment');
          const stabilised = (Date.now() - layersSeenAt) >= STABILIZE_MS;
          if (hasSegments || stabilised) {
            this._log(`Map loaded after reboot (segments=${hasSegments})`);
            return;
          }
        }
      } catch {
