...
class ValetudoDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    this._initServices(settings, sshKey);
    this._initState();
  ...
  _initState() {
    ...
    // Restore persisted snapshots so non-active floors are viewable after Homey restart
    this._restorePersistedSnapshots();
  }

  // Snapshot schema version — bump whenever the stored format changes so that
  // structurally incompatible snapshots are recognised and skipped.
  static get SNAPSHOT_VERSION() { return 2; }

  _restorePersistedSnapshots() {
    try {
      const stored = this.getStoreValue('map_snapshots');
      if (!stored || typeof stored !== 'object') return;

      if (stored.__version !== ValetudoDevice.SNAPSHOT_VERSION) {
        this.log('Snapshot version mismatch — restoring all entries as-is');
      }
      ...
      for (const [id, snap] of Object.entries(stored)) {
        ...
        if (snap && typeof snap === 'object') {
          this._mapSnapshots[id] = snap;
          count++;
          if (!Array.isArray(snap.layers) || snap.layers.length === 0) {
            this.log(`Warning: snapshot for "${id}" has no layer data (may render empty)`);
          }
        }
      }
      if (count > 0) this.log(`Restored ${count} map snapshot(s) from store`);
    } catch (err) {
      this.log('Could not restore persisted snapshots:', err.message);
    }
  }
  ...
  async _persistSnapshots() {
    try {
      ...
      let entries = Object.entries(this._mapSnapshots);
      if (entries.length > MAX_STORED) {
        const active = entries.filter(([id]) => id === activeId);
        const others = entries.filter(([id]) => id !== activeId);
        entries = [...active, ...others.slice(-(MAX_STORED - active.length))];
      }
      const toStore = Object.fromEntries(entries);
      toStore.__version = ValetudoDevice.SNAPSHOT_VERSION;
      await this.setStoreValue('map_snapshots', toStore);
    } catch (err) {
      this.log('Could not persist snapshots:', err.message);
    }
  }
  ...
  async onDiscoveryAvailable(discoveryResult) {
    ...
    this._applyNewHost(discoveryResult.address);
  ...
  _applyNewHost(address) {
    this._api.updateHost(address);
    this._ssh.updateConfig({ host: address });
    this._updateValetudoUrl(address);
    this.setSettings({ host: address }).catch(this.error);
  }
  ...
  requestFloorSnapshot(floorId) {
    // Non-active floor snapshots can only be obtained after the robot switches to that floor.
    // This is a no-op: the snapshot will be cached automatically the next time that floor is active.
    this.log(`Snapshot requested for "${floorId}" — will cache automatically on next switch`);
  }

  isFloorSnapshotLoading(floorId) {
    return false; // snapshots are not loaded on-demand
  }
  ...
  getFloorList() {
    ...
    return {
      floors: floors.map((f) => ({
        ...
        // Active floor always has a live map; non-active floors only if a snapshot is cached
        hasCachedMap: f.id === activeId || !!this._mapSnapshots[f.id],
      })),
